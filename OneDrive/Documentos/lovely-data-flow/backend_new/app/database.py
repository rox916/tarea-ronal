...
class Database:
    ...
    def connect(self):
        ...
        try:
            ...
            if db_url:
                ...
                print("⚠️ DATABASE_URL no configurada - modo simulación")
                self.connection = None
    ...
    def execute_query(self, query: str, params: tuple = None):
        if not self.connection:
            print("⚠️ No hay conexión a la base de datos - devolviendo datos simulados")
            return []
        try:
    ...
    def execute_insert(self, query: str, params: tuple = None):
        # Require a real DB connection for inserts in production/development.
        if not self.connection:
        ...
        try:
            ...
            with self.connection.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute(query, params)
                result = cursor.fetchone()  # ✅ Solo una vez
                self.connection.commit()
                if result and ("id" in result or len(result) > 0):
